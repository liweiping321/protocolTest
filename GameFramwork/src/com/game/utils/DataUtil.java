...
public class DataUtil {
	 ...
	 * @return
	 */
	public static String toHexString(short code, int maxSize){
		String hexStr = Integer.toHexString(code);
		StringBuilder result = new StringBuilder("0x");
		if(hexStr.length() <= maxSize){
			for(int length = hexStr.length(); length < maxSize; length ++){
				result.append("0");
			}
		}
		
		return result.append(hexStr).toString();
	}
	
	public static void main(String[] args) {
		for(int data = 0; data < 1000; data ++){
			System.err.println(toHexString((short)data, 3));
		}
	}
}
