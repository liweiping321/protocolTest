 ...
 * https://github.com/hueneburg/JavaLanguageFeatures.git
 * 
 */
public final class CSharpFormat
{
	public static String format(String format, Object... args) throws Exception {
		State state = State.Normal;
		if (format == null) {
			throw new Exception("format null");
		} else if (args == null) {
			...
			default:
				if (state == State.Normal) {
					...
					if ('0' <= c && c <= '9') {
						...
						throw new Exception("Unexpected character '" + c + "'. Number expected at " + index);
					}
				} else if (state == State.Closed) {
					throw new Exception("Unexpected character '" + c + "' at " + index);
				}
				break;
			}
		}
