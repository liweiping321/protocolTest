	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if (urlBean == null) {
				...
				response.getWriter().write(new HttpResponseMsg<String>(ResponseStatus.FAIL, "请求路径不存在：" + target).toJsonString());
			} else {
			    ...
			    if(!StringUtil.isEmpty(param)){
			    	...
			    	HttpResponseMsg<?> responseMsg=urlBean.getWebHandler().checkTCClientInfo(requestMsg.getSessionId());
			    	if(responseMsg!=null){
			    		response.getWriter().write(responseMsg.toJsonString());
			    	}else{
			    	...
			    	response.getWriter().write(new HttpResponseMsg<String>(ResponseStatus.FAIL, "请求参数为空!").toJsonString());
			    }
			...
			response.getWriter().write(new HttpResponseMsg<String>(ResponseStatus.FAIL, target).toJsonString());
		}
