...
import com.game.http.entity.TestBean;

public class HttpClient {

	private static final String CHARSET = "UTF-8";

	private static final RequestConfig requestConfig = RequestConfig.custom().setConnectionRequestTimeout(15000).setConnectTimeout(15000).setSocketTimeout(15000).build();
	...
	public static <V>  HttpResponseMsg<V> get(String url,HttpRequestMsg<V> requestMsg) throws Exception {
		CloseableHttpClient httpclient = HttpClients.createDefault();
		try {
	 ...
	 * @throws Exception 
	 */
	public static void postAsync(String url, List<NameValuePair> params, FutureCallback<HttpResponse> callback) throws Exception {
		CloseableHttpAsyncClient httpclient = HttpAsyncClients.createDefault();
		try {
			httpclient.start();
			final CountDownLatch latch = new CountDownLatch(1);
	...
	public static void main(String[] args)throws Exception {
		String url="http://localhost:8080/tc/api/up";
		HttpRequestMsg<TestBean> requestMsg=new HttpRequestMsg<TestBean>("111", new TestBean());
		
		HttpResponseMsg<TestBean> responseMsg=HttpClient.get(url, requestMsg);
		System.out.println(JSON.toJSONString(responseMsg.convertBody(TestBean.class)));
	}
}
